...
1. **识别增量**：新内容主要影响哪些章节：背景（3.1/3.2/3.3）、4.2 方案、4.3 代替方案、第五章实施例等。
2. **非破坏性合并**：以**追加或局部重写**为主，不推翻未涉及且用户未要求修改的章节。
3. **查新联动**：若增量改变了技术实质，判断是否需要**补充检索**并更新 3.1/3.3 / 区别论述。
4. **一致性**：合并后执行 `disclosure_self_check.md` 中的 **8.2、8.3** 快速检查；若涉及 **4.2.4 公式/4.2.5 参数**，须同步核对 **`disclosure_builder.md` §7.7**（符号表、维度下标、4.2.5 符号列同形）。
