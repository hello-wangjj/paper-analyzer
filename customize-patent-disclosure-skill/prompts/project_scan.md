...
## 图片与裸图目录（默认跳过；需识图时经 multimodal-vision）
...
- **例外（需要「看图」时）**：用户**点名**某图片路径，或某图**未**出现在任何已转换 Office 的 `_media` 中且对专利点明显关键时，**委派 `multimodal-vision` 子智能体**（Agent 工具，`subagent_type: multimodal-vision`）：
  - **给足上下文**：图片**绝对路径** + 具体问题，如「这是方案架构图，提取模块名、连接关系与数据流」「这是流程截图，按序提取步骤与判定分支」；**禁止**泛泛一句「分析这张图」。
  - **交叉核对**：子智能体返回的是报告推断；据此写入专利点材料表的技术事实，须与周边文本（图注、文档正文、代码）交叉核对后再采信。
  - **回退**：当前环境无该子智能体时，主代理直接读图分析。
